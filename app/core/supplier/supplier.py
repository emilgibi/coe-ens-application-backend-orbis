...
def validate_and_update_data(data, user_id, session_id):
    ...
    f"unmodified_{key}": str(value) for key, value in row.items()
        ...
        uploaded_date = clean_date(
            prefixed_row.get('uploaded_client_onboarding_date')
        )
        unmodified_date = clean_date(
            prefixed_row.get('unmodified_client_onboarding_date')
        )
        prefixed_row['uploaded_client_onboarding_date'] = uploaded_date
        prefixed_row['unmodified_client_onboarding_date'] = unmodified_date
...
async def process_excel_file(file_contents, client_id: str, user_id: str, user_email: str, session) -> Dict:
    try:
        logger.info(f"process_excel_file: {file_contents}")
        contents = await file_contents.read()
        excel_file = io.BytesIO(contents)
        df = pd.read_excel(excel_file)
        df = df.where(pd.notnull(df), "")  
        allowed_rows = get_settings().allow.general
        logger.info(f"allowed_rows: {allowed_rows}")
        ...
        validate_and_update_data(sheet_data, user_id, session_id)
        ...
        logger.exception(f"Unhandled exception in process_excel_file: {error}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
    ...
    try:
        ...
        reject_or_accept_others_query = (
            ...
            .values(
                ...
                uploaded_address=table_class.c.uploaded_address,
                uploaded_client_onboarding_date=table_class.c.uploaded_client_onboarding_date,
