...
async def process_excel_file(file_contents, current_user, session) -> Dict:
    try:
        ...
        allowed_rows = get_settings().allow.tprp
...
def generate_container_presigned_url(bucket_name, session_id_prefix, expiry_weeks=1):
    """
    Generate a pre-signed URL to list files under a session prefix in R2 (S3 compatible).

    :param bucket_name: Name of the R2 bucket (container).
    :param session_id_prefix: Prefix for objects (e.g., session_id/).
    :param expiry_weeks: Validity of URL.
    :return: Pre-signed URL for S3 list_objects_v2 API (simulates container SAS).
    """
    try:
        ...
        expiration = min(int(timedelta(weeks=expiry_weeks).total_seconds()), 604800)

        presigned_url = r2_client.generate_presigned_url(
            "list_objects_v2",
            Params={"Bucket": bucket_name, "Prefix": f"{session_id_prefix}/"},
        ...
        return {
            ...
            "sas_token": None  # Not applicable in R2 context
        }
...
async def r2_get_session_screening_status_static(
    session_id: str,
    session: AsyncSession = Depends(deps.get_session)
):
    try:
        ...
        if not rows:
            return {"error": "No matching session_id found"}

        # R2 Bucket name from ENV
        bucket_name = get_settings().r2_storage.storage_container_name
        session_sas = generate_container_presigned_url(
            bucket_name=bucket_name,
            session_id_prefix=session_id,
            expiry_weeks=2
        )

        logger.debug(f"R2 Presigned URL: {session_sas}")
